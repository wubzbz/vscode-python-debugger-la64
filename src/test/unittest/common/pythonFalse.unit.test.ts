...
suite('Python API Tests - useEnvironmentsExtension:false', () => {
    ...
    setup(() => {
        ...
        // Mock useEnvExtension to return false for this test suite
        sinon.stub(utilities, 'useEnvExtension').returns(false);
    ...
    teardown(() => {
        sinon.restore();
        // Reset useEnvironmentsExtension after each test
        (pythonApi as any)._useExt = undefined;
    });
    ...
    suite('resolveEnvironment', () => {
        test('Should resolve environment from path string', async () => {
            ...
            const mockResolvedEnv: ResolvedEnvironment = {
                id: 'test-env',
            ...
            mockPythonEnvApi.environments.resolveEnvironment.resolves(mockResolvedEnv);
            sinon.stub(PythonExtension, 'api').resolves(mockPythonEnvApi);
        ...
        test('Should resolve environment from Environment object', async () => {
            ...
            const expectedEnv = buildPythonEnvironment(pythonPath, '3.9.0');
